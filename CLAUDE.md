...
- `public/maps/` – Tiled-Maps (JSON, Tilesets eingebettet). Kollision über die Tile-Property `collides` im Layer `walls`, Spawnpunkte als Objekte im Layer `objects`. Beuten sind Punkt-Objekte vom Typ `loot` im Layer `objects` mit der string-Property `kind` (Schlüssel aus `LOOT`). Lichtzonen sind Rechtecke im Layer `lights` mit der float-Property `brightness` (0–1); alles außerhalb ist dunkel.
