...
- Map-Auswahl: `?map=demo` (Standard, die Demo-Map aus `docs/demo.md`), `?map=pier9` (die große Map) oder `?map=testmap`. Schilder sind Objekte vom Typ `sign`, ihr Name ist der Text.
- Sichtkegel der Wachen und Kameras liegen unter der Dunkelheit, aber kräftig genug (`BEAM_ALPHA`), dass sie durch die Dunkelheit im eigenen Sichtfeld wie Taschenlampenlicht lesbar sind und hinter der vollen Dunkelheit fast verschwinden: Gefahr zeigt sich, bevor sie gefährlich ist. (Phaser 4 hat keine Geometrie-Masken mehr, nur Mask-Filter; die sparen wir uns.)
- Spielgefühl: Der Spieler läuft 120 px/s (Schleichen 60), Wachen patrouillieren mit 70 und jagen mit 135, also schneller als er läuft. Bewegung hat Trägheit (`PLAYER_ACCELERATION`/`PLAYER_DECELERATION` je Tick auf `vx`/`vy` im Zustand; Anlaufen etwa 0,1 s). Die Szene hat drei Kameras: Welt (Zoom 1,5, Wärmebild-Filter), Beschriftungen im Weltraum (Zoom 1,5, ohne Filter; `label()`) und festes HUD (Zoom 1; `hud()`). Die Weltkamera folgt weich (`CAMERA_LERP`) mit Blick voraus (`LOOK_AHEAD`) und zoomt im Versteck leicht heran. Figuren (`Figure`) sind Schatten, Körper und Kopf in Blickrichtung mit Schrittwippen; Wachen zeigen „?“ beim Stutzen und „!“ mit Kamerawackeln beim Alarm. Lichtzonen haben weiche Ränder; Start blendet ein, Aufheben ploppt. Die Hinweiszeile sagt „Shift halten: schleichen“, sobald eine stehende Wache in Sicht und näher als `SNEAK_PROMPT_RANGE` ist.
...
- `scripts/` – Generatoren: `make-tileset.mjs` (Platzhalter-Tileset), `make-demo.mjs` (Demo-Map) und `make-pier9.mjs` (große Map); beide nutzen `tiled.mjs` (Gitter, Objekte, Writer). Ein Generator ist der erste Entwurf; danach ist die Tiled-Datei die Quelle.
